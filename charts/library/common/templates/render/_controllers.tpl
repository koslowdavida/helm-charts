...
{{- define "bjw-s.common.render.controllers" -}}
  ...
  {{- range $identifier := keys $enabledControllers -}}
    ...
    {{- else if eq $controllerObject.type "rollout" -}}
      {{- $rolloutObject := (include "bjw-s.common.lib.rollout.valuesToObject" (dict "rootContext" $rootContext "id" $identifier "values" $controllerObject "itemCount" (len $enabledControllers))) | fromYaml -}}
